...
const pageMeta = {
  ...
  stockdetail: ["Stock Profile", "Detailed stock data"],
  cryptodetail: ["Coin Profile", "Detailed cryptocurrency data"],
...
function renderSymbolPage() {
  ...
  $("allStocksTable").innerHTML = items.map(item => {
    ...
    return `
      <tr class="clickable-row ${changePct == null ? "" : (changePct >= 0 ? "tint-positive" : "tint-negative")}" data-symbol-open="${esc(item.symbol)}">
  ...
  document.querySelectorAll(".view-stock-button").forEach(button => {
    button.addEventListener("click", (e) => { e.stopPropagation(); openStock(button.dataset.symbol); });
  });
  document.querySelectorAll("#allStocksTable [data-symbol-open]").forEach(row => {
    row.addEventListener("click", () => openStock(row.dataset.symbolOpen));
  });
...
function getHiddenSectors() {
  try { return JSON.parse(localStorage.getItem("psxhub_hidden_sectors") || "[]"); }
  catch { return []; }
}
function setHiddenSectors(list) {
  localStorage.setItem("psxhub_hidden_sectors", JSON.stringify(list));
}
...
async function checkPremarketSignal() {
  try {
    ...
    if ("Notification" in window && Notification.permission === "granted") {
      new Notification("PSX Hub: Overnight markets pointing lower", {
...
function renderCryptoPage() {
  ...
  $("cryptoTable").innerHTML = items.map(c => {
    ...
    return `
    <tr class="clickable-row ${pct == null ? "" : (pct >= 0 ? "tint-positive" : "tint-negative")}" data-crypto-open="${esc(c.symbol)}">
  ...
  document.querySelectorAll("#cryptoTable [data-crypto-open]").forEach(row => {
    row.addEventListener("click", () => openCrypto(row.dataset.cryptoOpen));
  });
...
// =====================================================================
// Crypto Detail — clicking a coin opens its own profile page directly,
// with real stats (from the already-loaded coin list, no extra request)
// and a real candlestick chart (Yahoo Finance).
// =====================================================================

let currentCryptoSymbol = null;
let currentCryptoTimeframe = "1M";
let cryptoChartRequestSeq = 0;
let cryptoPriceChart = null, cryptoVolumeChartInstance = null;

function findCoin(symbol) {
  const upper = String(symbol || "").toUpperCase();
  return allCoins.find(c => (c.symbol || "").toUpperCase() === upper) || null;
}

async function openCrypto(symbol) {
  await go("cryptodetail");
  currentCryptoSymbol = symbol;
  currentCryptoTimeframe = "1M";
  document.querySelectorAll("#cryptoChartTimeframeGroup .chart-tf-btn").forEach(b => b.classList.toggle("active", b.dataset.tf === "1M"));

  const coin = findCoin(symbol);
  $("cryptoDetailName").textContent = coin ? coin.name : symbol;
  $("cryptoDetailSymbol").textContent = symbol.toUpperCase() + (coin?.market_cap_rank ? ` · Rank #${coin.market_cap_rank}` : "");

  const pct = coin?.price_change_percentage_24h;
  $("cryptoDetailPrice").textContent = coin?.current_price != null
    ? "$" + Number(coin.current_price).toLocaleString(undefined, { maximumFractionDigits: coin.current_price < 1 ? 6 : 2 })
    : "—";
  $("cryptoDetailChange").textContent = pct == null ? "24h change unavailable" : `${pct >= 0 ? "▲" : "▼"} ${Math.abs(pct).toFixed(2)}% (24h)`;
  $("cryptoDetailChange").className = pct == null ? "" : (pct >= 0 ? "positive" : "negative");

  $("cryptoDetailMetrics").innerHTML =
    metric("Market Cap", coin?.market_cap == null ? null : fmtLargeNumber(coin.market_cap)) +
    metric("24h Volume", coin?.total_volume == null ? null : fmtLargeNumber(coin.total_volume)) +
    metric("Market Cap Rank", coin?.market_cap_rank ?? null) +
    metric("Symbol", symbol.toUpperCase());

  loadCryptoChart(symbol, "1M");
}

function destroyCryptoCharts() {
  [cryptoPriceChart, cryptoVolumeChartInstance].forEach(c => { if (c) { try { c.remove(); } catch (e) {} } });
  cryptoPriceChart = cryptoVolumeChartInstance = null;
}

async function loadCryptoChart(symbol, timeframe) {
  currentCryptoTimeframe = timeframe || currentCryptoTimeframe;
  const statusEl = $("cryptoDetailChartStatus");
  if (!statusEl) return;
  statusEl.textContent = "Loading chart…";

  const seq = ++cryptoChartRequestSeq;
  const requestedTimeframe = currentCryptoTimeframe;

  try {
    const d = await getJSON(`/api/crypto/chart/${encodeURIComponent(symbol)}?timeframe=${encodeURIComponent(currentCryptoTimeframe)}`);
    if (seq !== cryptoChartRequestSeq) return;
    if (!d.available) {
      statusEl.textContent = d.note || "Chart data is not available for this coin yet.";
      destroyCryptoCharts();
      ["cryptoMainChart", "cryptoVolumeChart"].forEach(id => { if ($(id)) $(id).innerHTML = ""; });
      return;
    }
    statusEl.textContent = d.data_source ? `Real market data: ${d.data_source}` : "";
    requestAnimationFrame(() => {
      if (seq !== cryptoChartRequestSeq) return;
      if (currentCryptoTimeframe !== requestedTimeframe) return;
      renderCryptoChart(d);
    });
  } catch (error) {
    if (seq !== cryptoChartRequestSeq) return;
...
function renderCryptoChart(d) {
  if (typeof LightweightCharts === "undefined") {
    $("cryptoDetailChartStatus").textContent = "Chart library failed to load (check your connection and reload).";
    return;
  }
  destroyCryptoCharts();
  const priceEl = $("cryptoMainChart"), volEl = $("cryptoVolumeChart");
  if (!priceEl) return;

  try {
    cryptoPriceChart = LightweightCharts.createChart(priceEl, chartBaseOptions(priceEl));
    const candleSeries2 = cryptoPriceChart.addCandlestickSeries({
    ...
    const safeCandles = (d.candles || []).filter(x => [x.open, x.high, x.low, x.close].every(Number.isFinite));
    candleSeries2.setData(safeCandles);
    cryptoPriceChart.timeScale().fitContent();

    cryptoVolumeChartInstance = LightweightCharts.createChart(volEl, chartBaseOptions(volEl));
    const volumeSeries2 = cryptoVolumeChartInstance.addHistogramSeries({ priceFormat: { type: "volume" }, color: CHART_COLORS.up });
    volumeSeries2.setData(d.volume || []);
    cryptoVolumeChartInstance.timeScale().fitContent();

    const charts = [cryptoPriceChart, cryptoVolumeChartInstance];
    charts.forEach(c => {
    ...
    ["cryptoMainChart", "cryptoVolumeChart"].forEach(id => { if ($(id)) $(id).innerHTML = ""; });
    const statusEl = $("cryptoDetailChartStatus");
    if (statusEl) statusEl.textContent = "Could not render this chart (unexpected data). Try a different timeframe.";
  }
}

$("cryptoChartTimeframeGroup")?.addEventListener("click", (e) => {
  const btn = e.target.closest(".chart-tf-btn");
  if (!btn || !currentCryptoSymbol) return;
  document.querySelectorAll("#cryptoChartTimeframeGroup .chart-tf-btn").forEach(b => b.classList.remove("active"));
  btn.classList.add("active");
  loadCryptoChart(currentCryptoSymbol, btn.dataset.tf);
});
...
function renderScreenerData(d, { fromSaved = false } = {}) {
  const lastScanText = d.last_scan_at ? `Last scan: ${new Date(d.last_scan_at).toLocaleString()}` : "";
  const topStatus = $("screenerTopStatus");
  if (topStatus) {
    topStatus.textContent = lastScanText
      ? `${lastScanText}${d.stale ? " (saved result — press Run Screener to refresh)" : ""}${d.warming_up ? " • still warming up more symbols…" : ""}`
      : (d.warming_up ? "Warming up PSX data — results will fill in shortly." : "");
  }
  ...
  $("screenerResultCount").textContent = `${d.count} match${d.count === 1 ? "" : "es"} of ${d.scanned} scanned${lastScanText ? ` • ${lastScanText}` : ""}${d.stale ? " • saved result" : ""}${d.warming_up ? " • still warming up" : ""}`;
...
let screenerWarmupRetryTimer = null;

async function loadScreenerLast() {
  try {
    const d = await getJSON("/api/screener/last");
    if (!d.found) {
      const topStatus = $("screenerTopStatus");
      if (topStatus) topStatus.textContent = "No saved scan yet — press Run Screener.";
      return;
    }
    renderActiveFilterChips(d.criteria || {});
    renderScreenerData({ ...d.data, last_scan_at: d.saved_at, stale: true }, { fromSaved: true });
  } catch (error) {
    // Keep whatever the screener section already shows.
  }
}
...
async function runScreener(presetCriteria = null, { isRetry = false } = {}) {
  const criteria = presetCriteria || collectScreenerCriteria();
  renderActiveFilterChips(criteria);

  if (screenerWarmupRetryTimer) { clearTimeout(screenerWarmupRetryTimer); screenerWarmupRetryTimer = null; }
  ...
  try {
    ...
    renderScreenerData(d);

    // Some/all requested technical data was still loading server-side and
    // was handed off to a background warmer instead of blocking this
    // request. Re-run once automatically once it's had time to finish,
    // instead of leaving the user with an incomplete result and no signal.
    if (d.warming_up && !isRetry) {
      screenerWarmupRetryTimer = setTimeout(() => runScreener(criteria, { isRetry: true }), 20000);
    }
  } catch (error) {
    if (!isRetry) $("screenerResults").innerHTML = `<div class="error-panel">${esc(error.message)}</div>`;
  } finally {
    if (!isRetry) {
      btn.disabled = false;
      btn.textContent = old;
...
async function loadStockChart(symbol, timeframe) {
  ...
  // Guards against out-of-order responses: if the user clicks another
  // timeframe/stock before this request finishes, an older, slower
  // response arriving after the newer one used to overwrite it — the
  // chart would flash/settle on the wrong timeframe's data.
  const seq = ++chartRequestSeq;
  const requestedSymbol = symbol, requestedTimeframe = currentChartTimeframe;

  try {
    const d = await getJSON(`/api/stock/${encodeURIComponent(symbol)}/chart?timeframe=${encodeURIComponent(currentChartTimeframe)}`);
    if (seq !== chartRequestSeq) return; // superseded by a newer request
    if (!d.available) {
    ...
    requestAnimationFrame(() => {
      ...
      if (currentChartSymbol !== requestedSymbol || currentChartTimeframe !== requestedTimeframe) return;
      renderStockChart(d);
    });
  } catch (error) {
    if (seq !== chartRequestSeq) return;
...
function renderStockChart(d) {
  ...
  try {
    renderStockChartInner(d, priceEl, volEl, rsiEl, macdEl);
  } catch (error) {
    // A bad/edge-case candle set must never leave half-built chart panes
    // on screen (some panes drawn, others not) — that half-drawn state is
    // exactly what showed up as charts "glitching".
    destroyStockCharts();
    ["mainPriceChart", "volumeChart", "rsiChart", "macdChart"].forEach(id => { if ($(id)) $(id).innerHTML = ""; });
    const statusEl = $("chartStatus");
    if (statusEl) statusEl.textContent = "Could not render this chart (unexpected data). Try a different timeframe.";
...
function renderStockChartInner(d, priceEl, volEl, rsiEl, macdEl) {
...
window.addEventListener("resize", () => {
  const pairs = [
    ...
    [cryptoPriceChart, "cryptoMainChart"], [cryptoVolumeChartInstance, "cryptoVolumeChart"],
  ];
